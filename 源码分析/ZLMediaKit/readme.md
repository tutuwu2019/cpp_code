...
> 协议还是很难统一啊，比如说，不同的项目，项目配置的api 有差别、日志格式有差别 维护起来也不方便啊
...
> ZLM 中的 splitter 是 流式字节到协议帧/消息的边界切分器 + 分发器

以 RtspSplitter 为例

1. 定界(framing)
在TCP 字节流里找一条问政协议包的边界(包头/包体/长度前缀)
2. 粘包拆包
处理"半包+粘包+多包连发"
3. 协议分路
判断当前包是RTSP 信令还是RTP interleaved
4. 回调上抛
把完整包交给上层虚函数 onRTpPacket/onWholeRtspPacket



```text
RtspSplitter：拆包器（TCP字节流 → 完整RTSP包 / RTP包）
RtspSession：服务端会话（处理客户端发来的 RTSP 请求）
RtspPlayer：客户端拉流器（处理服务端返回的 RTSP 响应与媒体）
RtspPusher：客户端推流器（处理推流握手流程与发送RTP）

上面的四个类都围绕 RtspSplitter 的模版方法工作
1. 外层收到 socket 数据
2. 调 input(data,len)（来自 splitter 体系）
3. RtspSplitter::onSearchPacketTail 找边界（RTSP头 / $ RTP）
4. RtspSplitter::onRecvHeader 或 onRecvContent
5. 触发虚函数：
   1）onWholeRtspPacket(parser)（完整RTSP信令）
   2）onRtpPacket(data,len)（RTP interleaved）

差异只在第5步：不同子类实现不同业务语义。

场景A： 客户端拉流 RtspPlayer
RtspPlayer 发 OPTIONS/DESCRIBE/SETUP/PLAY
服务端回RTSP 响应(带 SDP、Transport、Session等)
RtspPlayer 收到数据后走 splitter:
   完整RTSP 响应 -> RTspPlayer::onWholeRtspPacket
   RTP over TCP -> RtspPlayer::onRtspPacket
解析SDP建轨道，接收RTP，送解码链路

角色分工：
RtspSplitter 负责包边界
RtspPlayer 负责响应状态机(下一步发什么)

场景B：客户端推流（RtspPusher）
流程例子
   RtspPusher 发 ANNOUNCE（带 SDP）-> SETUP -> RECORD
   服务端返回各阶段响应
   RtspPusher::onWholeRtspPacket 根据响应推进状态
   握手完成后持续发 RTP（TCP/UDP）

角色分工
   RtspSplitter 负责拆响应包
   RtspPusher 负责推流握手推进与发流控制

场景C：服务端接入（RtspSession）
你现在看的 RtspSession 就是这个：

流程例子
   客户端连到 ZLM，发 DESCRIBE 或 ANNOUNCE
   RtspSession::onRecv -> input(...)
   RtspSplitter 触发 RtspSession::onWholeRtspPacket
   RtspSession 内部分发到 handleReq_*
      拉流：DESCRIBE/SETUP/PLAY
      推流：ANNOUNCE/SETUP/RECORD
若 RTP over TCP，onRtpPacket 进入收包处理

角色分工
   RtspSplitter 做通用拆包
   RtspSession 做服务端协议业务（鉴权、找流、响应）
```
